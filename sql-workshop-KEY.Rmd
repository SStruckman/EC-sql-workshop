...
dragons_con = DBI::dbConnect(RSQLite::SQLite(),'dragons_simple.db')
...
```{r}
dbListTables(dragons_con)

dbListFields(dragons_con, "dragons")
```

We can read a table from the database right into R's `data.frame` format
```{r}
dragons_df = dbReadTable(dragons_con, "dragons")
```

... or execute queries in SQL syntax right within R by wrapping it in the `dbGetQuery()` function.
```{r}
dragons_query = dbGetQuery(dragons_con, 
           '
           SELECT * FROM dragons
           LIMIT 10 --show just first ten results
           '
           )
```
